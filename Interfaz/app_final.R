...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        id = "tabs",
        tabPanel("Resultados Tabulados", h4("Resultados de Actividad Enzimática por Grupo"), uiOutput("resultados_tabulados_ui")),
        tabPanel("Parámetros Cinéticos", h4("Parámetros Cinéticos Estimados por Grupo"), verbatimTextOutput("parametros")),
        tabPanel("Gráficas",
                 ...
                 fluidRow(
                   column(6, checkboxGroupInput("plotConc_groups", "Seleccionar Grupo(s):", choices = NULL)) # Checkbox
                 ),
                 h4("Gráfica de Actividad vs. Absorbancia Neta"), # Título de la primera gráfica
                 plotOutput("plotConcentration", height = "400px"),
                 hr(),
                 fluidRow(
                   column(12, h5("Opciones para 'Gráfica de Actividad vs. Tiempo'"))
                 ),
                 fluidRow(
                   column(6, checkboxGroupInput("plotAct_groups", "Seleccionar Grupo(s):", choices = NULL)), # Checkbox
                   column(6, checkboxInput("plotAct_show_sd", "Mostrar Desviación Estándar", value = FALSE)) # SD solo aquí
                 ),
                 h4("Gráfica de Actividad vs. Tiempo"),
                 plotOutput("plotActividad", height = "400px"),
                 hr(),
                 fluidRow(
                   column(12, h5("Opciones para 'Gráfica de Decaimiento Enzimático'"))
                 ),
                 fluidRow(
                   column(6, checkboxGroupInput("plotDecay_groups", "Seleccionar Grupo(s):", choices = NULL)) # Checkbox
                 ),
                 h4("Gráfica de Decaimiento Enzimático"),
                 plotOutput("plotDecay", height = "400px")
        )
      ),
      hr(),
...
server <- function(input, output, session) {
  ...
  datos_procesados <- eventReactive(input$calcular, {
    ...
    validate(need(!is.nan(OD_blanco_promedio), "No se encontraron datos válidos para 'Blanco' en OD1/OD2."), 
             need(!is.nan(OD_estandar_promedio), "No se encontraron datos válidos para 'Estandar' en OD1/OD2."))
  ...
  output$downloadReport <- downloadHandler(
    ...
    content = function(file) {
      ...
      tabla_data <- datos_procesados() # Ahora contiene Grupo, SD_U_L, Absorbancia_Neta, Tiempo_fermentacion_h
      params_texto <- parametros_cineticos_por_grupo() # Texto ya formateado por grupo
      grupos_disponibles_rep <- unique(tabla_data$Grupo) # Pasar los grupos para el RMD
      ...
      # (Se asume que 'reporte_template.Rmd' está en el mismo directorio que 'app.R')
      ...
      # Se pasan los datos brutos procesados y el texto de parámetros cinéticos.
      # El RMD se encargará de generar las tablas y gráficas por grupo.
      params <- list(
        datos_completos = tabla_data,
        resultados_cineticos = params_texto,
        grupos_disponibles_rep = grupos_disponibles_rep, # Necesario para iterar en el RMD
