glucosaUI <- function(id) {
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        id = ns("tabs"),
        tabPanel("Resultados Tabulados", h4("Concentración de Glucosa por Grupo"), uiOutput(ns("tablas_resultados"))),
        tabPanel("Análisis de Consumo", h4("Parámetros de Glucosa por Grupo"), uiOutput(ns("analisis_por_grupo"))),
        tabPanel("Gráficas",
                 h4("1. Curva de Calibración (Estándares)"),
                 plotlyOutput(ns("plot_calibracion")),
                 hr(),
                 h4("2. Absorbancia vs. Concentración de Glucosa"),
...
glucosaServer <- function(id, datos_crudos_r = reactive(NULL)) {
  moduleServer(id, function(input, output, session) {
    ...
    datos_analizados <- eventReactive(input$calcular, {
      ...
      df_tabla_resultados <- df_raw %>% filter(Tipo == "Muestra") %>% mutate(Grupo = substr(Muestra_ID, 1, 1), OD1_corregida = OD1 - od_blanco, OD2_corregida = OD2 - od_blanco, Glucosa_uM_OD1 = pmax(0, (OD1_corregida / slope) * input$DF), Glucosa_uM_OD2 = pmax(0, (OD2_corregida / slope) * input$DF)) %>% group_by(Grupo, Tiempo_fermentacion) %>% summarise(OD1_corregida_mean = mean(OD1_corregida, na.rm = TRUE), OD2_corregida_mean = mean(OD2_corregida, na.rm = TRUE), Glucosa_uM_OD1_mean = mean(Glucosa_uM_OD1, na.rm = TRUE), Glucosa_uM_OD2_mean = mean(Glucosa_uM_OD2, na.rm = TRUE), Glucosa_uM_mean = mean(c(Glucosa_uM_OD1, Glucosa_uM_OD2), na.rm = TRUE), Glucosa_uM_sd = sd(c(Glucosa_uM_OD1, Glucosa_uM_OD2), na.rm = TRUE), .groups = 'drop') %>% arrange(Grupo, Tiempo_fermentacion)
    ...
    # --- 2. Pestañas de Tablas y Análisis (Esta parte es correcta y no se modifica) ---
    ...
    observe({ req(datos_analizados()); df_tabla <- datos_analizados()$tabla_resultados; for (g in unique(df_tabla$Grupo)) { local({ grupo_local <- g; output[[paste0("tabla_res_", grupo_local)]] <- renderTable({ df_tabla %>% filter(Grupo == grupo_local) %>% mutate(Tiempo_h = Tiempo_fermentacion / 60) %>% select(`Tiempo (min)` = Tiempo_h, `OD1 (Abs)` = OD1_corregida_mean, `OD2 (Abs)` = OD2_corregida_mean, `Conc. glucosa OD1 (µM)` = Glucosa_uM_OD1_mean, `Conc. glucosa OD2 (µM)` = Glucosa_uM_OD2_mean, `Glucosa Promedio (µM)` = Glucosa_uM_mean, `Desv. Est. (µM)` = Glucosa_uM_sd) %>% mutate(across(where(is.numeric), ~ sprintf("%.3f", .))) }, striped = TRUE, hover = TRUE, bordered = TRUE) }) } })
    ...
    # Gráfico de calibración (sin cambios)
    ...
    output$plot_diagnostico_muestras <- renderPlotly({
      ...
      if (isTRUE(input$show_mean_diag) && isTRUE(input$show_sd_diag)) {
        fig <- fig %>% add_ribbons(data = df_sumarizado, x = ~OD_corregida_mean,
                                   ymin = ~Glucosa_uM_mean - Glucosa_uM_sd, ymax = ~Glucosa_uM_mean + Glucosa_uM_sd,
                                   color = ~Grupo, legendgroup = ~Grupo, opacity = 0.2, line = list(width = 0),
                                   name = ~paste(Grupo, "SD"), showlegend = FALSE, hoverinfo = 'none')
      }
      if (isTRUE(input$show_mean_diag)) {
        fig <- fig %>% add_trace(data = df_sumarizado, x = ~OD_corregida_mean, y = ~Glucosa_uM_mean, type = 'scatter', mode = 'lines',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "Promedio"),
                                 line = list(dash = 'dash'), hoverinfo = 'text', text = ~paste("Grupo:", Grupo, "<br>Tiempo:", Tiempo_fermentacion))
      }
      if (isTRUE(input$show_OD1_diag)) {
        df_od1 <- filter(df_long, replica == "OD1")
        fig <- fig %>% add_trace(data = df_od1, x = ~OD_corregida, y = ~Glucosa_uM, type = 'scatter', mode = 'markers',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "OD1"),
                                 marker = list(symbol = 'circle'), showlegend = FALSE,
                                 hoverinfo = 'text', text = ~paste("Muestra:", Muestra_ID))
      }
      if (isTRUE(input$show_OD2_diag)) {
        df_od2 <- filter(df_long, replica == "OD2")
        fig <- fig %>% add_trace(data = df_od2, x = ~OD_corregida, y = ~Glucosa_uM, type = 'scatter', mode = 'markers',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "OD2"),
                                 marker = list(symbol = 'square'), showlegend = FALSE,
                                 hoverinfo = 'text', text = ~paste("Muestra:", Muestra_ID))
      }
    ...
    output$plot_consumo_con_sd <- renderPlotly({
      ...
      if (isTRUE(input$show_mean_cons) && isTRUE(input$show_sd_cons)) {
        fig <- fig %>% add_ribbons(data = df_sumarizado, x = ~Tiempo_fermentacion,
                                   ymin = ~Glucosa_uM_mean - Glucosa_uM_sd, ymax = ~Glucosa_uM_mean + Glucosa_uM_sd,
                                   color = ~Grupo, legendgroup = ~Grupo, opacity = 0.2, line = list(width = 0),
                                   name = ~paste(Grupo, "SD"), showlegend = FALSE, hoverinfo = 'none')
      }
      if (isTRUE(input$show_mean_cons)) {
        fig <- fig %>% add_trace(data = df_sumarizado, x = ~Tiempo_fermentacion, y = ~Glucosa_uM_mean,
                                 type = 'scatter', mode = 'lines+markers',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "Promedio"),
                                 hoverinfo = 'text', text = ~paste("Glucosa Media:", round(Glucosa_uM_mean, 2)))
      }
      if (isTRUE(input$show_OD1_cons)) {
        df_od1 <- filter(df_long, replica == "OD1")
        fig <- fig %>% add_trace(data = df_od1, x = ~Tiempo_fermentacion, y = ~Glucosa_uM, type = 'scatter', mode = 'markers',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "OD1"),
                                 marker = list(symbol = 'circle'), showlegend = FALSE,
                                 hoverinfo = 'text', text = ~paste("Muestra:", Muestra_ID))
      }
      if (isTRUE(input$show_OD2_cons)) {
        df_od2 <- filter(df_long, replica == "OD2")
        fig <- fig %>% add_trace(data = df_od2, x = ~Tiempo_fermentacion, y = ~Glucosa_uM, type = 'scatter', mode = 'markers',
                                 color = ~Grupo, legendgroup = ~Grupo, name = ~paste(Grupo, "OD2"),
                                 marker = list(symbol = 'square'), showlegend = FALSE,
                                 hoverinfo = 'text', text = ~paste("Muestra:", Muestra_ID))
      }
