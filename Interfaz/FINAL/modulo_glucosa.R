...
glucosaUI <- function(id) {
  ns <- NS(id)
  fluidPage(
    titlePanel("Análisis de Consumo de Glucosa"),
    sidebarLayout(
      sidebarPanel(
        ...
        hr(),
        h4("2. Configuración"),
        numericInput(ns("DF"), "Factor de dilución (DF):", value = 1),
        hr(),
...
glucosaServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    datos_analizados <- eventReactive(input$calcular, {
      ...
      muestras_long <- df_raw %>% filter(Tipo == "Muestra") %>% 
        ...
        mutate(
          ...
          Valor = pmax(0, (OD_corregida / slope) * input$DF) # Para glucosa es OD / slope
        )
    ...
    # --- Outputs para UI de Glucosa ---
    output$tablas_resultados <- renderUI({ req(datos_analizados()); lapply(unique(datos_analizados()$muestras_sumarizado$Grupo), function(g) { tagList(h5(paste("Resultados Glucosa - Grupo", g)), tableOutput(session$ns(paste0("tabla_res_", g))), hr()) }) })
    observe({ req(datos_analizados()); df <- datos_analizados()$muestras_sumarizado; for(g in unique(df$Grupo)) { local({ g_local <- g; output[[paste0("tabla_res_", g_local)]] <- renderTable({ df %>% filter(Grupo == g_local) %>% select(Tiempo_fermentacion, `Glucosa Media (µM)` = Valor_mean, `DE (µM)` = Valor_sd) }) }) } })
