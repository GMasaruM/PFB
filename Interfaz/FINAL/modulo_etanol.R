...
etanolUI <- function(id) {
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput(ns("volumen_final"), "Volumen Final de Producción (mL):", value = 1000, min = 1, step = 1),
...
etanolServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({ req(datos_procesados()); df_final <- datos_procesados()$df_final; lapply(unique(df_final$Grupo), function(g) { output[[paste0("tabla_", g)]] <- renderTable({ df_final %>% filter(Grupo == g) %>% select(Muestra_ID, Tiempo_fermentacion, OD1, OD2, OD_promedio, OD_desv_std, Concentracion_OD1, Concentracion_OD2, Concentracion_promedio, Concentracion_desv_std) %>% rename("Muestra ID" = Muestra_ID, "Tiempo (min)" = Tiempo_fermentacion, "Abs. OD1" = OD1, "Abs. OD2" = OD2, "Abs. Promedio" = OD_promedio, "Desv. Est." = OD_desv_std, "Conc. Etanol 1 (%)" = Concentracion_OD1, "Conc. Etanol 2 (%)" = Concentracion_OD2, "Conc. Etanol Promedio (%)" = Concentracion_promedio, "Desv. Est. (%)" = Concentracion_desv_std) %>% mutate(across(where(is.numeric), ~round(., 4))) }, striped = TRUE, hover = TRUE, bordered = TRUE) }) })
      ...
      datos %>%
        ...
        summarise(
          ...
          `Rendimiento (% g/g)` = {
            ...
            g_etanol <- (max_conc / 100) * input$volumen_final * 0.789 # Densidad etanol ~0.789 g/mL
